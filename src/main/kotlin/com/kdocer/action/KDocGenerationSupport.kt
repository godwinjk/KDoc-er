...
import com.kdocer.merge.KDocMerger
 ...
 * action. Generation, the existing-KDoc [policy][ExistingKDocPolicy] (merge/keep/replace)
...
object KDocGenerationSupport {
     ...
     */
    fun computeText(project: Project, declaration: KtDeclaration, policy: ExistingKDocPolicy): String? {
        if (!declaration.isValid) return null
        val generator = generatorFor(project, declaration) ?: return null
        return KDocMerger.resolve(declaration.docComment?.text, generator.generate(), policy)
    }
    ...
    fun applyTo(project: Project, declaration: KtDeclaration, policy: ExistingKDocPolicy) {
    ...
    fun documentFile(project: Project, file: KtFile, policy: ExistingKDocPolicy) {
